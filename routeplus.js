...
module.exports.routeplus = function (parent) {
  ...
  obj.onWebUIStartupEnd = function () {
    ...
    var x =
      '<span id="plugin_routePlusSettings" style="display: block;"><a onclick="pluginHandler.routeplus.openSettings();">RoutePlus</a></span>';
    ld.innerHTML += x;
  ...
  obj.updateUserRdpLinks = function () {
    if (userinfo == null) {
      setTimeout(pluginHandler.routeplus.updateUserRdpLinks, 100);
      return;
    }
    meshserver.send({
      action: 'plugin',
    ...
    if (pluginHandler.routeplus.myRDPLinks != null) {
      if (currentNode != null && pluginHandler.routeplus.myRDPLinks[currentNode._id] != null) {
        ...
        var holderC = Q('p10html3'),
          holder = holderC.querySelectorAll('.p10html3left')[0];
  ...
  obj.hook_agentCoreIsStable = function (myparent, gp) {
    // check for remaps when an agent logs in
    obj.db
      .getMyComputerByNode(myparent.dbNodeKey)
      .then((mys) => {
        if (mys.length) {
          var my = mys[0];
          obj.db
            ...
            .then((maps) => {
              ...
              if (maps.length && onlineUsers.indexOf(my.user) !== -1) {
                // if we have a mapping and our user is online, map it
  ...
  obj.serveraction = function (command, myparent, grandparent) {
    switch (command.pluginaction) {
      ...
      case 'updateMapPort':
        ...
        var upUser = null;
        obj.db
          .update(command.mid, { localport: command.port })
          .then(() => {
      ...
      case 'rdpLinkUpdate':
        var upUser = null;
        obj.db
          .update(command.mid, { rdplink: command.rdplink })
          .then(() => {
            return obj.db.get(command.mid);
          })
          .then((mObj) => {
      ...
      case 'getRdpLinks':
        obj.db
          .getRdpLinksForUser(command.user)
          .then((links) => {
            var x = { action: 'plugin', plugin: 'routeplus', method: 'setUserRdpLinks', data: links };
